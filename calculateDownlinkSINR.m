function [sinr_macro_cells,sinr_small_cells,devices_small_dist,devices_macro_dist] = calculateDownlinkSINR(devices,white_noise,small_cells_x_pos,small_cells_y_pos,subcarrier_spacing,devices_x_pos,devices_y_pos,macro_cell_radiation,small_cell_radiation,macro_cells_x_pos,macro_cells_y_pos,devices_macro_dist,path_loss_macro_cells,channel_gain_macro_cells,sinr_macro_cells,devices_small_dist,path_loss_small_cells,channel_gain_small_cells,sinr_small_cells,device_throughput_demands,device_rb_demands,macro_cells,small_cells,downlink_subcarriers)
    ...
    for j = 1:devices
        % loop through all macro cells 
        for i=1:macro_cells
            sinr_numerator = macro_cell_radiation*channel_gain_macro_cells(j,i);
            all_other_macro_channel_gains_and_powers = 0;
            for temp = 1:macro_cells
                if temp ~= i 
                    all_other_macro_channel_gains_and_powers = all_other_macro_channel_gains_and_powers + macro_cell_radiation*channel_gain_macro_cells(j,temp);
                end
            end
            for temp = 1:small_cells
                    all_other_macro_channel_gains_and_powers = all_other_macro_channel_gains_and_powers + small_cell_radiation*channel_gain_small_cells(j,temp);
            end
        ...
        for i=1:small_cells
             sinr_numerator = small_cell_radiation*channel_gain_small_cells(j,i);
             all_other_small_channel_gains_and_powers = 0;
             for temp = 1:small_cells
                 if temp ~= i 
                     all_other_small_channel_gains_and_powers = all_other_small_channel_gains_and_powers + small_cell_radiation*channel_gain_small_cells(j,temp);
                 end
             end
             for temp = 1:macro_cells
                    all_other_small_channel_gains_and_powers = all_other_small_channel_gains_and_powers + macro_cell_radiation*channel_gain_macro_cells(j,temp);
             end
